...
sudo cp -f juiceiot-mosquitto-docker.service /lib/systemd/system/juiceiot-mosquitto-docker.service && \
echo "Setting service permissions" && \
sudo chmod 644 /lib/systemd/system/juiceiot-mosquitto-docker.service && \

echo "Enabling service" && \
sudo sh $SYSTEMCTL_SCRIPT enable juiceiot-mosquitto-docker.service && \
echo "Starting/restarting service" && \
sudo sh $SYSTEMCTL_SCRIPT restart juiceiot-mosquitto-docker.service && \
